...
> **Canon-revalidation notice (2026-09-07):** The 74 translated units on the historical checkpoint and the immutable v0.1.0/Zenodo artifacts remain available for provenance, but only the repaired contiguous prefix is currently semantically accepted. That prefix now covers OLP-0001–OLP-0010: 163/163 choice groups, comprising 68 formal invariants and 95 explicit reversible supported-provisional choices, with zero unsupported, needs-revision, or contentious rows. Forward translation remains frozen while repair continues at OLP-0011. This is a prefix result, not a corpus-success claim. See [the full notice](provenance/CANON_REVALIDATION_NOTICE.md) and [the repair ledger](provenance/SEMANTIC_REPAIR_CHOICES.jsonl).

## Current scope

The v0.1.0 reader tranche is the **complete Sets chapter**: seven source units, six sections, three original diagrams, definitions, proofs and exercises. It is not the whole book. That release snapshot contains 21 source-aligned translations. Current `main` contains 74 reviewed source-aligned translations, leaving 648 of the frozen 722-unit inventory; the Relations, Functions, Size of Sets, Arithmetization and Infinite chapters have complete source translations, including both alternate non-enumerability presentations, both constructions of the reals, Dedekind's philosophical reflection and the closure proof of Schröder-Bernstein. Propositional Logic includes the complete shared Proof Systems survey sequence—sequent calculus, natural deduction, tableaux and axiomatic deduction—and now continues into the detailed Sequent Calculus chapter through its finite-side semantics, exact logical and structural rule schemata, Cut, inductively defined derivations and worked proof-tree construction. Its integrated reader is not yet built, and none of these later sources is included in the Sets PDF's layout acceptance. See `provenance/CURRENT_MAIN_QA.json` for the branch/release distinction.
...
Release files include the two PDF profiles, an editable-source ZIP, and a SHA-256 inventory. See `provenance/RELEASE_QA.json` for exact accepted bytes, source coverage, reproducibility and visual checks. The entire English source is preserved in `upstream/`; translated source is separate in `translation/`. Git text normalization is disabled for `upstream/**`, so all 722 public source blobs retain the manifest-addressed bytes. `provenance/UPSTREAM-EOL-REPAIR.json` records the one-time repair of 84 historical newline-only Git representations and the staged 722/722 verification.
...
The included OFL font bundle and relocatable generator remain a **portable candidate, not a release asset**. At the OLP-0010 checkpoint it built reproducibly in both Naskh and Nastaliq under the shared TeX mutex, and all 29 generated pages passed a complete local visual review. The generator isolates all 65 explicit English terminology bridges so their internal LTR order survives RTL paragraph layout; that review caught and corrected an apostrophe-sensitive `Russell's Paradox` heading reversal before acceptance. The candidate now contains the entire semantically accepted seven-unit Sets chapter, but this evidence does not replace the immutable reference-v3 release or establish cross-platform byte reproducibility. See [`PORTABLE_CANDIDATE_QA.json`](provenance/PORTABLE_CANDIDATE_QA.json). All source, font, candidate-input and reference identities can be checked without TeX:
