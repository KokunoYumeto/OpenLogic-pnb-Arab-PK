...
The v0.1.0 reader tranche is the **complete Sets chapter**: seven source units, six sections, three original diagrams, definitions, proofs and exercises. It is not the whole book. That release snapshot contains 21 source-aligned translations. Current `main` contains 38 reviewed source-aligned translations, leaving 684 of the frozen 722-unit inventory; the Relations and Functions chapters now have complete source translations, and Size of Sets is translated through the Schroder-Bernstein theorem and the alternate set-theoretic enumeration presentation. Their integrated readers are not yet built, and none of these later sources is included in the Sets PDF's layout acceptance. See `provenance/CURRENT_MAIN_QA.json` for the branch/release distinction.
